...
Render a movie of gate/latch closure dynamics for the pair_3101_binder
PYR1+LCA complex, for use in a research talk.

This script targets the t=0-30 ns window (gate_closure_pair3101_0_30ns_PL.xtc)
of pair_3101_binder, chosen after scanning gate_latch_timeseries.xvg across
~38 binder sequences for a cleaner, more visually obvious closure event than
seq14_binder's messy multi-modal one (seq14_binder had a closing transition
immediately followed by a wide partial-reopening excursion -- see git history
of this file for that version). Smoothed (10-point rolling average)
gate-latch distance for pair_3101_binder: open baseline ~0.95-1.06 nm over
t=0-2 ns (peak 1.06 nm right at t=0), a sharp, clean, essentially monotonic
closing transition down to ~0.73 nm over t=2-4 ns, then a remarkably stable
closed state in a tight ~0.70-0.78 nm band (std ~0.02-0.03 nm) from t=4 ns
all the way through t=33 ns -- a real settled closed state, not breathing
between open/closed like seq14_binder. A second reopening starts around
t~34 ns, outside this window. This is a single clean closure transition,
better material for "clearly shows the gate closure event" than the
seq14_binder window this script previously rendered.

Mechanistic story being illustrated: in binders, the gate loop (resi 84-90)
and latch (resi 114-118) close down over the bound ligand (LIG); this script
renders every Nth frame of the trajectory as a fast (non-ray-traced) PNG with
a FIXED camera, then stitches the PNGs into an MP4 with ffmpeg, so that any
...
Other environment variable overrides (all optional):
    GATE_LATCH_STRIDE        raw-trajectory-frame stride between rendered
                              output frames (default 2; the source
                              trajectory has 801 raw frames at 37.5 ps
                              spacing spanning t=0-30 ns, so stride 2 gives
                              ~401 output frames, i.e. ~40.1 s at 10 fps --
                              0.75 ns of sim time per second of video, the
                              same slow pace that worked well for the prior
                              seq14_binder 0-25ns version, landing in the
                              requested ~30-40 s total range)
    GATE_LATCH_MAX_FRAMES    if set, caps the number of OUTPUT frames loaded
...
Design decisions worth knowing about if you're re-running or adapting this:
  ...
  1. Removing rigid-body drift before rendering. The trajectory is a
     ...
     rotation. Per the task spec, all loaded states are fit to state 1
     (t=0 ns for this 0-30 ns window) with cmd.intra_fit() BEFORE any
     camera/orient calls, so the movie shows only internal conformational
     change, not tumbling.
     The fit selection ("core_fit_sel") is protein CA atoms EXCLUDING the
     gate (84-90), latch (114-118), and the Lb7a5/C-terminal recoil region
     (148-166; see CLAUDE.md "Key domain conventions") -- i.e. we fit on
     the stable scaffold and deliberately leave the flexible/functional
     loops we actually want to see moving OUT of the least-squares fit, so
     their motion isn't partially averaged into the global superposition.
  ...
  2. Fixed camera. cmd.orient() + cmd.zoom() are called exactly ONCE, on
     ...
     (orient/zoom/turn/move) is called again anywhere in the per-frame
     render loop -- only cmd.frame(state) changes, which just swaps which
     already-fitted coordinate set is displayed under the same camera.

  3. On-screen running time label. Rather than doing PyMOL view-matrix
     decoding (fragile, version-dependent) to place a fixed 2D HUD label,
     this script reuses the same atomic-coordinate-only placement trick
     already used in pymol_renders/medoid_comparison.py: a label anchor
     pseudoatom is placed at a fixed point in the (now drift-corrected)
     model coordinate frame, offset outward from the ligand along the
     protein-core -> ligand vector. Because the whole coordinate frame is
     stable across frames (that's the point of step 1) and the camera never
     moves (step 2), a single fixed-in-model-space point stays fixed on
     screen for the whole movie. The label TEXT is re-issued every frame
     (cheap) with the real simulation time in ns, computed from frame index
     * stride * 37.5 ps + T0_NS offset (NOT from PyMOL's movie-frame
     counter or the ray/opengl clock). T0_NS = 0.0 for this 0-30 ns window
     (it was 40.0 for the original 40-500 ns full-trajectory version of
     this script).
  ...
  4. Speed. All output frames are captured with PyMOL's OpenGL renderer
     ...
     Ray-tracing every frame individually would take far too long for this
     deliverable, even at this window's much smaller frame count (~401
     frames vs. ~1023 for the original full-trajectory version).
"""
...
# t=0-30 ns closure-event window for pair_3101_binder, extracted by the
# coordinator from the same Protein_LIG index group convention used for the
# earlier seq14_binder windows. IMPORTANT: atom count (topology/atom
# order/count -- 2956 atoms, confirmed matching this sequence's own
# medoid_PL.pdb via grep count of ATOM records) is DIFFERENT from
# seq14_binder's 2961 atoms -- pair_3101_binder has different mutations, so
# do not assume atom counts carry over across sequences. Lives in this
# repo's scratch dir, not OneDrive, since it's a derived intermediate.
XTC = os.path.join(
    REPO_ROOT,
    "pymol_renders",
    "scratch_traj",
...
def remove_rigid_body_drift():
    """Fit every loaded state onto state 1 using only stable-core CA atoms
    (gate/latch/Lb7a5/recoil loops excluded), so apparent motion in the
    movie is real conformational change, not whole-protein tumbling.
    intra_fit applies the resulting rigid transform to the WHOLE object
    (all atoms, all selections) per state, so the ligand and gate/latch
    loops are carried along correctly relative to the rest of the protein.
    """
...
def add_time_label_anchor():
    """Place a fixed-in-model-space label anchor just outside the ligand,
    along the (stable-core -> ligand) direction at state 1, mirroring the
    atomic-coordinate-only placement trick in
    pymol_renders/medoid_comparison.py::add_pocket_label. No camera/view
    matrix decoding required: because the coordinate frame is drift
    corrected (remove_rigid_body_drift) and the camera never moves, a
...
def set_fixed_camera(anchor):
    ...
    called anywhere else in this script (see docstring point 2).

    buffer is larger than the medoid_comparison.py reference (5 A) as a
    safety margin, though for pair_3101_binder state 1 (t=0) sits right at
    (or near) the open-baseline peak (~0.95-1.06 nm gate-latch distance,
    per the coordinator's smoothed timeseries), and the rest of the window
    (closing transition to ~0.73 nm, then a stable closed band of
    ~0.70-0.78 nm through t=30 ns) stays at or inside that span -- unlike
    the earlier seq14_binder window, this sequence shouldn't need extra
    headroom beyond the t=0 framing snapshot, since t=0 is close to the
    widest-open moment in this window. Kept the larger buffer anyway since
    it's harmless (just slightly more zoomed out) and I still have no way
    to render and check for clipping myself -- check the smoke test and
    tighten this back toward 5 if you want a closer framing.
    """
    cmd.frame(1)
