#!/usr/bin/env python3
"""
build_new_ligand_feat_table.py

Builds a separate, schema-matched feature table for the 4 new-ligand test
sequences (cdca/glca/lca3s/lca_001_binder) -- NOT merged into
feat_table_500ns.xlsx. These sequences have no experimental Binder_score
(there's no prior measurement for "does this variant bind CDCA/GLCA/LCA3S",
that's what the model is being used to predict), so they're kept as a
separate held-out validation set: train on the 95-sequence LCA cohort via
feat_table_500ns.xlsx as usual, then run the trained model's predict/
predict_proba on this table.
...
generated by the extra core/tail R-score and pocket-hydration runs (see
this session's notes) that were never run for these 4 sequences before.
"""
import os
import argparse
import pandas as pd
